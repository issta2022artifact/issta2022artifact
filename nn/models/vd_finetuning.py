...
import torch
from src.datas.samples import SampleBatch
...
class VulDetectModel(LightningModule):
    ...
    def training_step(self, batch: SampleBatch,
    ...
    def validation_step(self, batch: SampleBatch,
                        batch_idx: int) -> torch.Tensor:  # type: ignore
        # [n_method; n_classes]
        logits = self(batch.ast_graphs, batch.statements,
                      batch.statements_per_value_flow,
    ...
    def test_step(self, batch: SampleBatch,
                  batch_idx: int) -> torch.Tensor:  # type: ignore
        # [n_method; n_classes]
        logits = self(batch.ast_graphs, batch.statements,
                      batch.statements_per_value_flow,
